...
TextareaController = (function(superClass) {
  ...
  TextareaController.prototype.catchQuery = function() {
    ...
    //alert(subtext.substring(subtext.length-1));
    var act = true;
    if(subtext.indexOf("@Agregar Variable") > -1 ){
        this.$inputor.val(content.replace('@Agregar Variable',''));
         content = this.$inputor.val();
        caretPos = this.$inputor.caret('pos', {
          iframe: this.app.iframe
        });
        subtext = content.slice(0, caretPos);
        agregarVariableFuncion();
    }
         
    
     try{
       var str = $('#alta_regla_procedure').val().replace(/@/g, '');
        str = str.replace(/:/g,'.'); 
        str = str.replace(/#/g,'');
        
        var syntax = esprima.parse(str);
        
        $('#validador_sintactico_div').addClass('alert-success'); 
        $('#validador_sintactico_div').removeClass('alert-danger'); 
        
        $('#validador_sintactico').html('Sintaxis correcta');
        $('#alta_guardar_regla').prop('disabled', false);
        
       
    }catch(err){
        
        $('#alta_guardar_regla').prop('disabled', true);
        
        $('#validador_sintactico_div').removeClass('alert-success'); 
        $('#validador_sintactico_div').addClass('alert-danger'); 
        
        
        $('#validador_sintactico').html("Error en sintaxis "+err.message);
    }

    
    
    if(subtext.substring(subtext.length-1) ===":"){
        if(this.at === ":"){
            //
            //var data1 = [{"name":"no_tienes_nada"}];
           ...
           var  r = /@((\w|\d|\.)+(\s:)*)+/g;
            ...
            var match;
            var s ='';
            while ((match = r.exec(subtext)) != null) {
             
              s = match[0];
            }
            
               var num_proceso = '';
     $( "#select_procesos option:selected" ).each(function(index,item) {
            num_proceso =$(item)[0].value;
      });
    
            
           var params = {exp:s.substring(0,s.length-1),idu_proceso: num_proceso};
            
            act = false;           
            var url =  "/incentivos-web/getOptions";
            var _this =this;
...
View = (function() {
  ...
  View.prototype.render = function(list) {
    ...
    for (i = 0, len = list.length; i < len; i++) {
      ...
      if(this.context.query === null){
          this.context.query.text = "";
      }
...
$.fn.destruirsfx = function() {
    alert(this.controllers);
};
